...
| ID | Requirement |
| --- | ----------- |
| ECHO-01 | Chinese-пул активирован: `tuning.chinese.enabled=true` в `System/Config.json`. Поле `ru_hint` карточки прокидывается в `[hint]`-строку для LLM (через `InjectedEcho.hint_text` или эквивалент), т.к. Silero `v5_5_ru` не озвучивает иероглифы — LLM получает русскую смысловую подсказку наряду с китайской фразой. Pre-rendered wav-озвучка по `audio_id` — вне scope (deferred). |
| ECHO-02 | Слой A — тематический мост: `EchoGate._score_match` матчит теги карточки против множества `{acc.themes} ∪ {ключевые слова кластеров из tuning.memory.theme_clusters}`, а не против сырого транскрипта. Семантически совпадающие реплики («мне одиноко» → тема `одиночество`) дают ненулевой score для карточек с тегом `одиночество`. Чистые образы-теги (не входящие ни в один кластер) сохраняются для редких буквальных совпадений. |
| ECHO-03 | Слой A — окно истории: матчинг идёт против взвешенного окна последних N реплик (зритель + Адам), не одной сырой реплики. Реплики Адама несут внутреннюю лексику (технофлора, память, симбиоз), совпадающую с тегами. Размер окна и веса — в Config. Текущая реплика зрителя имеет максимальный вес, предыдущие — затухание. |
| ECHO-04 | Слой B — мягкий вероятностный движок: жёсткий порог `match_threshold` заменён моделью `final = thematic_match × weight × recency_decay`; кандидаты выше низкого пола выбираются взвешенно-случайно (не top-1 по обрыву). RNG остаётся seedable для тестов. Редкость обеспечивается cooldown'ами (`global_cooldown_turns`, `per_echo_cooldown_days`), а не порогом. |
| ECHO-05 | Слой D — разнообразие: gate не инжектит дважды за сессию карточки одного семантического кластера (анти-повтор на уровне группы тегов, не только per-id cooldown). Состояние кластеров за сессию хранится в `EchoGate` или `SessionAccumulator`. |
| ECHO-06 | Слой D — починка mood: устранена мёртвая проводка `mood`/`mood_block`. Либо `mood` вычисляется реально (из `adam_state`/сцены) и передаётся в gate, либо инертный `mood_block`-фильтр удалён из расчёта, чтобы не создавать ложного ощущения работающего фильтра. `adam_state`, передаваемый в `maybe_inject`, либо используется, либо убирается из сигнатуры. |
| ECHO-07 | Слой C — спонтанный канал: независимый низковероятный путь инжекта по внутренним сигналам (длинная пауза зрителя / `acc.turn_count` / N-й turn), выбор карточки по `weight` без тематического матча. Вероятность и триггеры — в Config. Соблюдает те же cooldown'ы и анти-повтор (ECHO-05). |
| ECHO-08 | Все новые числовые параметры (размер окна, веса recency, пол выбора, вероятность спонтанного канала, пороги) добавлены в `System/Config.json` И описаны в `System/Config.schema.json` (Config-First инвариант). Тесты покрывают: тематический матчинг (ECHO-02/03), вероятностный выбор (ECHO-04), анти-повтор кластера (ECHO-05), спонтанный канал (ECHO-07), активацию Chinese (ECHO-01). |
