...
- **WhisperX + Jetson CUDA — ВСЕГДА через Docker.** `ctranslate2` для aarch64 в pip-wheel **CPU-only** (нет CUDA build в PyPI). Native Python ASR (`Speech.ASR_WhisperX`) молча fallback'нется на CPU → ~12с/transcribe → realtime UX рушится (см. Test 13 в `tests/RESULTS.md`). Canonical path — Docker контейнер `adam-asr-whisperx` в `compose.yaml` (NVIDIA runtime, healthcheck assert `device=cuda`). Systemd unit `adam-asr-whisperx.service` — тонкий wrapper над `docker compose up -d adam-asr-whisperx`. **НЕ запускать** `python -m Speech.ASR_WhisperX` напрямую (даже для диагностики) — port collision на 8095 + silent CPU fallback. Диагностика CUDA: `scripts/adam_asr_cuda_check.sh`. VAD в пайплайне — WebRTC VAD (`webrtc_vad.py`, CPU-only, без PyTorch), не Silero.
...
- Не запускать native `python -m Speech.ASR_WhisperX` (CPU silent fallback на aarch64) — только через Docker compose
