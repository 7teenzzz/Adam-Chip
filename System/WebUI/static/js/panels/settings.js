  ...
  // ── ASR config ───────────────────────────────────────────────────────────────
  {
    ...
    fields: [
      ...
      { key: "wake_words",       label: "Wake words",       type: "text",
        hint: "через запятую: адам,дохлый" },
      { key: "wake_word_required", label: "Wake word обязателен", type: "bool",
        hint: "включить в exhibition mode" },
      { key: "barge_in_enabled", label: "Barge-in (перебивание)", type: "bool",
        hint: "агент замолкает если зритель начинает говорить" },
      { key: "endpointing_ms",   label: "Endpointing (мс)", type: "number",
        hint: "200–450 рекомендуется" },
      { key: "timeout_sec",      label: "Таймаут (с)",      type: "number" },
    ],
  },
  ...
  // ── Audio ─────────────────────────────────────────────────────────────────────
  {
    ...
    fields: [
      ...
      { key: "vad_threshold", label: "Порог VAD (СКО)", type: "number",
        hint: "400–800 · выше = только громкий звук" },
