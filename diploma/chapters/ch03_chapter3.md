...
Языком оркестрации выбран Python — он позволяет быстро собирать и перестраивать логику приложения. Операторская часть реализована как веб-интерфейс на FastAPI (порт 8080); отдельный сервис просмотра логов работает на порту 8083 независимо от основного оркестратора (подробнее — раздел 3.2.7).

Стек образует три уровня. Вычислительный: Jetson, CUDA, контейнеры, llama.cpp. Прикладной: оркестратор, память, сборщик контекста, речевые модули. Исполнительный: связь приложения с микроконтроллером и технофлорой (рисунок 2).

![Рисунок 2 — Программный стек нейроагентного приложения](../assets/diagrams/3_2_2_stack.svg)
...
![Рисунок 3 — Четыре уровня памяти нейроагентного приложения](../assets/diagrams/3_2_4_memory.svg)
...
### 3.2.7. Операторский веб-интерфейс

Операторская среда включает три браузерных интерфейса: встроенная панель микроконтроллера ESP32-S3 для мониторинга аппаратного слоя, основной интерфейс оркестратора на порту 8080 и автономный сервис логов на порту 8083. Разделение последних двух намеренно: даже при сбое оркестратора оператор сохраняет доступ к журналу событий и статусам сервисов.

Микроконтроллер ESP32-S3 предоставляет собственную диагностическую панель, доступную непосредственно по его IP-адресу. Она отображает ключевые статусы аппаратного слоя: состояние сети (IP-адрес, уровень сигнала Wi-Fi), параметры видеопотока (частота кадров, задержка захвата), профиль микрофона и статус аудиовыхода PCM5102A, показания датчиков освещённости. Из той же панели доступен прямой просмотр видеопотока с камеры и ручное управление аудиопотоком (рисунок 4).

![Рисунок 4 — Встроенная панель управления ESP32-S3: статусы камеры, аудио и сенсоров](../ui-screenshots/ESP-Web-UI.png)

Основной интерфейс оркестратора организован как одностраничное приложение с боковой навигацией из восьми разделов: «Чат», «Настройки», «Модели», «Подсистема», «Сервисы», «Личность агента», «Метрики», «Логи». Верхняя полоса постоянно отображает статус каждого компонента — оркестратора, языковой модели, синтеза и распознавания речи, визуальной модели, камеры, микрофона, мотрики — и текущие задержки по четырём ключевым стадиям цикла.

Раздел «Чат» — основная рабочая страница во время показа. Центральная колонка отображает историю диалога в реальном времени: реплики зрителя и ответы агента с временны́ми метками. Рядом — кадр с камеры и автоматическое описание сцены от визуальной модели. Правая колонка показывает поток событий шины с типами и метками времени (рисунок 5).

![Рисунок 5 — Страница «Чат»: история диалога, кадр камеры и поток событий шины](../ui-screenshots/Chat.png)

Вкладка «Аналитика» в разделе «Метрики» агрегирует показатели апробации по тем же направлениям, что и методика оценки в разделе 3.4: удержание роли и нормативность (стабильность персоны, фильтрация контента, устойчивость структуры), память и темпоральная связность (активация памяти посетителей, накопление дневника, межсессионная память), интерактивность и инициатива (длина диалога, точность команд микроконтроллеру, самостоятельность проактивных реплик). В нижней полосе отображаются задержки NLP-конвейера: среднее, минимальное и максимальное время для каждой стадии — распознавания речи, языковой модели и синтеза речи (рисунок 6).

![Рисунок 6 — Вкладка «Аналитика»: показатели апробации по разделам 3.4.2–3.4.4 и задержки NLP-конвейера](../ui-screenshots/metrics-analytics.png)

Раздел «Метрики» фиксирует временны́е характеристики каждого цикла взаимодействия. Сводная строка показывает среднее, минимальное и максимальное время для каждой стадии по всем накопленным обращениям. Развёрнутая таблица содержит каждое обращение: реплику зрителя, ответ агента и задержки по каждой стадии. Под таблицей — сырой лог событий (рисунок 7).

![Рисунок 7 — Страница «Метрики»: задержки по стадиям и таблица обращений](../ui-screenshots/metrics.png)

Раздел «Настройки» предоставляет редактор параметров без перезапуска. Поля организованы по подсистемам: параметры агента, конфигурация языковой модели, голос синтеза речи, пороги памяти, пробуждное слово. Изменения сохраняются в `Config.json` и применяются в следующем цикле взаимодействия. Механизм позволяет калибровать поведение прямо во время показа — например, снизить порог пробуждного слова в шумном выставочном зале (рисунок 8).

![Рисунок 8 — Страница «Настройки»: горячая перенастройка параметров без перезапуска](../ui-screenshots/settings.png)

Сервис просмотра логов (порт 8083) — отдельный постоянно активный процесс. Он читает данные из общей шины событий напрямую и предоставляет таблицу статусов сервисов с цветовой индикацией, поток событий с полными JSON-телами и журнал systemd для любого из сервисов инсталляции (рисунок 9). Доступность при падении оркестратора — ключевое требование: именно в этот момент диагностика наиболее необходима.

![Рисунок 9 — Сервис просмотра логов (порт 8083): статусы сервисов и поток событий](../ui-screenshots/LogViewer.png)
...
![Рисунок 11 — Диаграмма состояний цикла взаимодействия зрителя с инсталляцией](../assets/diagrams/3_3_5_states.svg)
...
Параметры пяти режимов тестирования приведены в приложении А. Состав и объём корпуса данных, собранного за период апробации, — в приложении Б.
...
Примеры разметки по четырём репрезентативным сессиям приведены в приложении В, сводные значения всех четырёх показателей — в приложении Г.
...
Примеры оценки по четырём сессиям приведены в приложении Д, сводные значения показателей CRS, LMRR, SCS — в приложении Е.
...
Примеры оценки интеракционности по четырём сессиям приведены в приложении Ж, разбивка задержек по этапам — в приложении И, сводные показатели блока — в приложении К.
...
Сводная оценка по всем четырём блокам приведена в приложении Л.
