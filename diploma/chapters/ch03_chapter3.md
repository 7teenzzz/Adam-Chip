...
| 45 | 3 | 2 156 | 2 156 | 5 312 | Техническая заглушка | ЯМ недоступна в момент цикла; ответ из резервного пула без генерации |
...
| Задержка ответа (полный цикл) | Отдельный цикл | 13 202 мс | 1 034 мс | 68 775 мс | p95 = 28 500 мс; этап генерации ЯМ (~6 с) — основное узкое место; задержка в художественном контексте воспринимается как характерная «пауза раздумья» агента |
...
| Временная эффективность | Задержка ответа | Среднее — 13 202 мс; p95 = 28 500 мс | Этап генерации ЯМ (~6 с) — основное узкое место; пиковые задержки свыше 30 с нарушают эффект присутствия | Задержка на верхней границе допустимого для выставочной среды; воспринимается частью зрителей как характерная «пауза раздумья» |
