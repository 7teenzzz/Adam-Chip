...
*Третий слой — семантическая память* представляет собой консолидированную долгосрочную запись в виде дневника (`diary.md`). Сюда входят имена постоянных посетителей (с указанием истории встреч), названия «цепляющих» тем, которые часто возникают, и интервальные суммари, созданные языковой моделью на основе анализа серий эпизодов. Консолидация может быть срабатываемой мгновенно (для особо значимых эпизодов) или происходить в фоновом режиме (например, раз в сутки). Таким образом информация из эпизодической памяти не просто хранится, а преобразуется в смыслосодержащие суммари.

*Четвёртый слой — поисковая память* представляет собой индексированное хранилище эпизодов, позволяющее быстро находить релевантные прошлые встречи по ключевым словам из текущего диалога. В текущей реализации используется TF-IDF индекс (волна 1). В перспективе планируется расширение на основе семантических векторов (embedding-based retrieval через FAISS), что позволит находить эпизоды не только по совпадению слов, но и по смысловой близости.
