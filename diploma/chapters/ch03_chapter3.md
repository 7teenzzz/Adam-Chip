...
Конфигурация AIIM не зашита в код, а читается из `Tuning.json` при каждом turn-е через модуль `tuning.py::TuningStore`. Это даёт возможность менять Δ-приоритеты, состояния активности и уровни зрелости аспектов в реальном времени, без перезапуска оркестратора. На текущем этапе модуляция выполняется вручную (для калибровки персонажа на этапе развёртывания), но та же инфраструктура служит основой для рефлексивного уровня AIIM: модуль метакогниции `se` может в перспективе анализировать паттерны взаимодействия и корректировать конфигурацию автоматически — это направление дальнейшей разработки.
...
*Четвёртый слой — поисковая память* представляет собой индексированное хранилище эпизодов, позволяющее быстро находить релевантные прошлые встречи по ключевым словам из текущего диалога. В текущей реализации используется BM25 — статистический алгоритм ранжирования документов по релевантности к запросу, оценивающий частоту общих слов с поправкой на их редкость в корпусе. Опционально подключается векторный поиск через FAISS (Facebook AI Similarity Search) — библиотеку для быстрого поиска ближайших соседей в пространстве числовых векторов. На текущем этапе векторы строятся методом TF-IDF (Term Frequency — Inverse Document Frequency, оценка значимости слова в документе относительно корпуса), что позволяет искать по лексической близости. В перспективе TF-IDF-векторы будут заменены на семантические эмбеддинги от языковой модели, что даст возможность находить эпизоды не только по совпадению слов, но и по смысловой близости фраз.
...
Ключевую роль здесь выполняет слой действий (`System/adam/action.py`, класс `ActionLayer`). Его задача — выделить
