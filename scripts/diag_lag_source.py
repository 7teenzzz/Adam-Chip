...
def main(argv: list[str]) -> int:
    ...
    if not windows:
        print()
        print("No data. Enable diagnostic (hot, no restart):")
        print('  curl --noproxy "*" -X POST http://127.0.0.1:8080/api/diag/lag/toggle \\')
        print('       -H "Content-Type: application/json" -d \'{"enabled": true}\'')
        print()
        print("Then say 'Адам' + ask 3-5 questions (do NOT respond in REPLY window —")
        print("we want to capture the audio envelope WITHOUT user speech).")
        print()
        print("Then re-run this script. After, disable to stop the flood of events:")
        print('  curl --noproxy "*" -X POST http://127.0.0.1:8080/api/diag/lag/toggle \\')
